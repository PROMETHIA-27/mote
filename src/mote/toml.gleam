...
external fn get_internal(
  Section,
  List(String),
) -> Result(dynamic.Dynamic, GetError) =
  "tomerl" "get"
...
pub fn get(section: Section, path: List(String)) -> Result(Value, GetError) {
  ...
  dyn_value(value)
  |> result.map_error(fn(_) { InvalidValue })
}
...
fn dyn_value(dyn: dynamic.Dynamic) -> Result(Value, List(dynamic.DecodeError)) {
  use <- result.lazy_or({
    maybe_num(dyn)
    |> result.map(MaybeNumber)
  })
  use <- result.lazy_or({
    dynamic.bit_string(dyn)
    |> result.map(Binary)
  })
  use <- result.lazy_or({
    dynamic.bool(dyn)
  ...
  use <- result.lazy_or({
    dyn
    |> dynamic.list(dyn_value)
    |> result.map(ValueList)
  })
  use <- result.lazy_or({
    dyn
    |> dynamic.map(dynamic.bit_string, dyn_value)
    |> result.map(Table)
  })
...
fn maybe_num(
  dyn: dynamic.Dynamic,
) -> Result(MaybeNumber, List(dynamic.DecodeError)) {
  use <- result.lazy_or({
    dynamic.int(dyn)
    |> result.map(Integer)
  })
  ...
  use <- result.lazy_or({
    ...
    use <- iff(
      is_atom(dyn, "infinity"),
      Infinity
      |> Ok,
    )
    use <- iff(
      is_atom(dyn, "negative_infinity"),
      NegativeInfinity
      |> Ok,
    )
    Error([])
...
external fn is_atom(dynamic.Dynamic, String) -> Bool =
  "mote@toml@erl" "is_atom"
